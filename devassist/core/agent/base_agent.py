...
class BaseAgent(ABC):
    """
    Abstract base class for all agents in the DevAssist framework.
    ...
    def __init__(self, name: Optional[str] = None, **kwargs):
        ...
        logging.debug(f"Agent '{self.name}' initialized with ID {self.id}")
    ...
    def update_state(self, **kwargs) -> None:
        ...
        logging.debug(f"Agent '{self.name}' state updated: {kwargs}")
    ...
    def log_action(self, action: str, details: Dict[str, Any]) -> None:
        ...
        logging.debug(f"Agent '{self.name}' performed action: {action}")
