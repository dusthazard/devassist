"""
Orchestrator module for the DevAssist framework.
...
from devassist.core.agent import BaseAgent, HybridAgent
from devassist.core.memory import ShortTermMemory, LongTermMemory
...
class AgentOrchestrator:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        logging.info("DevAssist Orchestrator initialized and ready")
    ...
    def execute_task(self, task: str, mode: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Log the task
        if mode == "multi":
            logging.info(f"Processing task in multi-agent mode: {task}")
        else:
            logging.info(f"Processing task in single-agent mode: {task}")
        ...
        if execution_time > 10:
            logging.info(f"Task completed after {execution_time:.2f}s - complex task")
        else:
    ...
    def _load_config(self, config_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if merged_config.get("agent", {}).get("mode") == "multi":
                logging.info("Configured for multi-agent execution mode")
            
            return merged_config
        except Exception as e:
            logging.error(f"Error loading config file: {e}")
            logging.info("Reverted to default configuration. Performance may not be optimal.")
            return default_config
    ...
    def _create_primary_agent(self) -> HybridAgent:
        ...
        name = agent_config.get("name", "devassist")
        ...
        logging.info(f"Primary agent created with {len(enabled_tools)} tools available")
        
        # Create specialized agents if in multi mode
        if mode == "multi" or mode == "auto":
            self._create_specialized_agents(agent)
            logging.info("Specialized agents created for multi-agent execution")
    ...
    def _create_short_term_memory(self) -> ShortTermMemory:
        ...
        logging.debug(f"Initializing short-term memory with capacity {capacity}")
    ...
    def _create_long_term_memory(self) -> Optional[LongTermMemory]:
        ...
        if not enabled:
            logging.info("Long-term memory disabled. System will forget everything after each session.")
            return None
        ...
        if storage_path:
            logging.debug(f"Long-term memories will be stored at: {storage_path}")
        else:
            logging.debug("Long-term memories will be stored in the default location")
    ...
    def _create_specialized_agents(self, primary_agent: HybridAgent) -> None:
        ...
        for role in roles:
            ...
            logging.debug(f"Added {role} agent to system")
        
        logging.info(f"Created {len(roles)} specialized agents for collaborative execution")